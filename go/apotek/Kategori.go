...
func Kategori(c *gin.Context) {
	...
	reqBody := JKategoriRequest{}
	jKategoriResponse := JKategoriResponse{}
	jKategoriResponses := []JKategoriResponse{}
	...
	logFile = logFile + "Kategori_" + dateNow + ".log"
	...
	if helper.ValidateHeader(bodyString, c) {
		if err := c.ShouldBindJSON(&reqBody); err != nil {
			...
			if errorMessage != "" {
				dataLogKategori(jKategoriResponses, username, errorCode, errorMessage, totalRecords, totalPage, method, path, ip, logData, allHeader, bodyJson, c)
				return
			}
			...
			if checkAccessVal != "1" {
				dataLogKategori(jKategoriResponses, username, errorCodeSession, errorMessageSession, totalRecords, totalPage, method, path, ip, logData, allHeader, bodyJson, c)
				return
			}
			...
			if method == "INSERT" {
				...
				var cekKategori int
				queryCheck := fmt.Sprintf("SELECT COUNT(1)  FROM db_kategori WHERE kategori = '%s'", kategori)
				...
				if cekKategori > 0 {
					errorMessage = "Kategori Obat sudah terdaftar"
					dataLogKategori(jKategoriResponses, username, errorCode, errorMessage, totalRecords, totalPage, method, path, ip, logData, allHeader, bodyJson, c)
					return
				}
				...
				dataLogKategori(jKategoriResponses, username, "0", "Insert berhasil", totalRecords, totalPage, method, path, ip, logData, allHeader, bodyJson, c)
				return
				...
				dataLogKategori(jKategoriResponses, username, "0", "Delete berhasil", totalRecords, totalPage, method, path, ip, logData, allHeader, bodyJson, c)
				return
				...
				totalRecords = 0
				totalPage = 0
				query := fmt.Sprintf("SELECT COUNT(1) AS cnt FROM db_kategori %s", queryWhere)
				...
				for rows.Next() {
					...
					jKategoriResponses = append(jKategoriResponses, jKategoriResponse)
				...
				dataLogKategori(jKategoriResponses, username, "0", errorMessage, totalRecords, totalPage, method, path, ip, logData, allHeader, bodyJson, c)
				return
			} else {
...
func dataLogKategori(jKategoriResponses []JKategoriResponse, username string, errorCode string, errorMessage string, totalRecords float64, totalPage float64, method string, path string, ip string, logData string, allHeader string, bodyJson string, c *gin.Context) {
	if errorCode != "0" {
		helper.SendLogError(username, "Kategori", errorMessage, bodyJson, "", errorCode, allHeader, method, path, ip, c)
	}
	returnKategori(jKategoriResponses, errorCode, errorMessage, logData, totalRecords, totalPage, c)
}

func returnKategori(jKategoriResponses []JKategoriResponse, errorCode string, errorMessage string, logData string, totalRecords float64, totalPage float64, c *gin.Context) {
