...
func Login(c *gin.Context) {
	...
	if helper.ValidateHeader(bodyString, c) {
		if err := c.ShouldBindJSON(&reqBody); err != nil {
			...
			statusAdmin := 0
			role := ""
			query := fmt.Sprintf("SELECT COUNT(1) AS cnt FROM db_login WHERE username = '%s'", username)
			if err := db.QueryRow(query).Scan(&countLogin); err != nil && err != sql.ErrNoRows {
			...
			if countLogin == 0 {
				...
				query := fmt.Sprintf("SELECT IFNULL(password, '') password, IFNULL(status, 0) status FROM db_login WHERE username = '%s'", username)
				if err := db.QueryRow(query).Scan(&passwordDB, &statusAdmin); err != nil && err != sql.ErrNoRows {
					errorMessage = fmt.Sprintf("Error running %q: %+v", query, err)
					dataLogLogin(username, paramKey, role, errorCode, errorMessage, method, path, ip, logData, allHeader, bodyJson, c)
					return
				}
